...
;
// returns u8x16
static __m128i vec_i8v32n_count_m128(size_t size, int8_t *src, int8_t value)
{
    size_t units = size / 16;
    __m128i *p = (void*)src;
...
int8_t vec_i8v32n_count_i8(size_t size, int8_t *src, int8_t value)
{
    ...
    results = _mm_adds_epu8(results, _mm_srli_esi128(results, 1));
    results = _mm_adds_epu8(results, _mm_srli_esi128(results, 2));
    results = _mm_adds_epu8(results, _mm_srli_esi128(results, 4));
    results = _mm_adds_epu8(results, _mm_srli_esi128(results, 8));
